...
class Single_dataset_eph(data.Dataset):
    def __init__(
            self,
            data_root,
            image_size,
...
class Single_dataset_domain(data.Dataset):
    def __init__(
            self,
            data_root,
            image_size,
        ...
        self.data_size = len(self.rgb_images)
    ...
    def __getitem__(self, index):
        while True:
            try:
                rgb_path = self.rgb_images[index]
                if self.split == 'train':
                    rgb_path2 = self.rgb_images[index % self.data_size]
                    domain_rgb = Image.open(rgb_path2).convert("RGB")
                filename = rgb_path.name
                rgb = Image.open(rgb_path).convert("RGB")
                size = rgb.size
                break
        ...
        if self.split == 'train':
            rgb = self.transform(rgb)
            domain_rgb = self.transform(domain_rgb)
        elif self.split == 'test':
            rgb = self.transform_test(rgb)
        else:
            raise NotImplementedError
        rgb = F2.to_tensor(rgb)
        if self.split == 'train':
            domain_rgb = F2.to_tensor(domain_rgb)
        if self.normalize_to_neg_one_to_one:
            rgb = normalize_to_neg_one_to_one(rgb)
            if self.split == 'train':
                domain_rgb = normalize_to_neg_one_to_one(domain_rgb)
        return {'image': rgb, 'domain_image': domain_rgb,'img_name': filename, 'ori_size': (size[1], size[0])}
